...
function LoadMultipleUsers(userCount){
    ...
    .then(data => {
        ...
        for(let i = 0; i < data.results.length; i++){
            let fn = '<div id="firstName">' + data.results[i].name.first + '</div>'
            let ln = '<div id="lastName">' + data.results[i].name.last + '</div>'
            let phone = '<div id="phone">' + data.results[i].phone + '</div>'
...
function loadGender(gender,userCount){
    ...
    .then(data => {
        ...
        for(let i=0; i<data.results.length; i++){
            ...
            let gendr = '<div>' + data.results[i].gender + '</div>'
            let img = '<img src="' + data.results[i].picture.large + '">'
            
            temp = temp + fn + ln + phone + gendr + img
